...
// Description:This file is implements the apb write and read sequence
// Date       :04/03/2026
//
//***************************************//
...
`ifndef _APB_WR_SEQ
`define _APB_WR_SEQ
class apb_wr_seq extends uvm_sequence#(apb_transaction);

// declaring handle transaction for request,response and factory refistraction   
   apb_transaction transaction_req_h;
   apb_transaction transaction_res_h;
   `uvm_object_utils_begin(apb_wr_seq)
...
// constructor for the apb write and read seq   
   function new(string name="apb_wr_seq");
      super.new(name);
   endfunction:new

// main task for the write and read sequence execution   
   task body();
      int i;
